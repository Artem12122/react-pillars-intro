...
//   return (
//     <div>
//       {/* <p>ID Nation: {props.ID_Nation}</p> */}
//       <p>Nation: {props.Nation}</p>
//       {/* <p>ID Year: {props.ID_Year}</p> */}
//       <p>Year: {props.Year}</p>
//       <p>Population: {props.Population}</p>
//       <p>Slug Nation {props.Slug_Nation}</p>
//     </div>
//   )
// }

function Populations({ID_Nation, Nation, ID_Year, Year, Population, Slug_Nation}) {
  return (
    <div>
...
function arrDecodSwapi(arr, name){
  return arr.length > 0 && arr.map((item, index) => (
    <div key={index}>
      <a href={item}>{name} {arr.length === 1 || index +1}</a>
    </div>
  ))
}

function Swapis({name, haight, mass, hair_color, skin_color, eye_color, birth_year, gender, homeworld, filmsArr, speciesArr, vehiclesArr, starshipsArr, created, edited, url}) {
  return <div>
    <h3>Name: {name}</h3>
    <p>Haight: {haight}</p>
    <p>Mass: {mass}</p>
    ...
    {birth_year !== "unknown" && <p>Birth year: {birth_year}</p>}
    {gender !== "n/a" && gender !== "none" && <p>Gender: {gender}</p>}
    <a href={homeworld}>URL homeworld {name}</a>

    {arrDecodSwapi(filmsArr, "Film")}
    {arrDecodSwapi(speciesArr, "Species")}
    {arrDecodSwapi(vehiclesArr, "Vehicles")}
    {arrDecodSwapi(starshipsArr, "Starships")}

    <p>Created: {created}</p>
    <p>Edited: {edited}</p>
    <a href={url}>URL {name} Swapi</a>
  </div>
}

function Swapi() {
  return arrSwapi.map(item => (
    <Swapis
      key={item.name}
      name={item.name}
      haight={item.height}
      ...
      filmsArr={item.films}
      speciesArr={item.species}
      vehiclesArr={item.vehicles}
      starshipsArr={item.starships}
...
function Episodes({name, air_date, characters}) {
  return (
    <div>
      <h3>Name: {name}</h3>
      <p>Date: {air_date}</p>
...
function Episode() {
  return arrEpisodes.map(item => 
    <Episodes
    key = {item.air_date}
    name = {item.name}
    air_date = {item.air_date}
    // characters = {item.characters.map((item, index) =>
    //   <li key={index}>{item.name}</li>
    // )}
...
function Characters({name, gender, image, episode}) {
  return (
    <div>
      <h3>Name: {name}</h3>
      <p>Gender: {gender}</p>
